...
from shapeup_geometry.utils.misc import get_device
...
def flow_sample(
    ...
    for i, t in enumerate(
        ...
        timestep_tensor = timestep_tensor.expand(latent_model_input.shape[0])
        noise_pred = diffusion_model.forward(
            latent_model_input, timestep_tensor, visual_cond, caption_cond, label_cond, shape_cond
        ).sample
...
def flow_sample_separate_cfg(
    ...
    steps: int,
    eta: float = 0.0,
    guidance_scale_visual: float = 3.0,
    guidance_scale_shape: float = 3.0,
    do_classifier_free_guidance: bool = True,
    generator: Optional[torch.Generator] = None,
    device: torch.device = "cuda:0",
    disable_prog: bool = True,
    ...
    for i, t in enumerate(
        ...
        # perform guidance
        if do_classifier_free_guidance:
            noise_pred_uncond, noise_pred_visual, noise_pred_both = noise_pred.chunk(3)
            noise_pred = noise_pred_uncond + guidance_scale_visual * (
                noise_pred_visual - noise_pred_uncond
