...
@shapeup_geometry.register("dinov2-clip-encoder")
class Dinov2CLIPEncoder(BaseVisualEncoder, ModelMixin):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.zero_uncond_embeds:
            ...
            with_dino = self.cfg.with_dino
            with_clip = self.cfg.with_clip
            if with_dino:
                if self.cfg.globals_only:
                    self.empty_image_embeds_dino = torch.zeros(
                        (self.cfg.n_views, 1, 1024)
                    ).detach()
                else:
                    self.empty_image_embeds_dino = torch.zeros(
            ...
            if with_dino and with_clip:
                ...
                self.empty_image_embeds = self.empty_image_embeds_dino
            else:
              self.empty_image_embeds = self.empty_image_embeds_clip  
    ...
    def encode_image_clip(
        ...
        **kwargs,
        ...
        camera_embeds = None
        test_text = "An image in botero style"
        ...
        if self.cfg.encode_camera and camera_embeds is not None:
            ...
            text_output = self.clip_model.text_model(test_text)
            vision_outputs = self.clip_model.vision_model(
                pixel_values=rearrange(
                    pixel_values.to(self.clip_model.device), "B N C H W -> (B N) C H W"
                ),
    ...
    def encode_image(
        ...
        **kwargs,
        ...
        if with_dino and with_clip:
            ...
            visual_embeds = clip_embeds
        else:
            visual_embeds = dino_embeds
