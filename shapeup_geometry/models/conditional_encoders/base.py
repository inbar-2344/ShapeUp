...
class BaseVisualEncoder(BaseModule):
    ...
    def forward(self, batch, **kwargs):
        assert (
            "input_image" in batch or "mvimages" in batch
        ), "image or mvimages is required for visual embeds"
        if batch["input_image"].dim() == 5:
            bs = batch["input_image"].shape[0] * batch["image"].shape[1]
        else:
            bs = batch["input_image"].shape[0]

        if random.random() < self.cfg.empty_embeds_ratio:
            if "input_image" in batch or "image_embeds" in batch:
            ...
            if "input_image" in batch:
                if self.cfg.encode_camera:
                    visual_embeds = self.encode_image(
                        batch["input_image"], cameras=batch["c2w"]
                    )
                else:
                    visual_embeds = self.encode_image(batch["input_image"])
