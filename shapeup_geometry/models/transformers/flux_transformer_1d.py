...
from shapeup_geometry.utils.base import BaseModule
...
@shapeup_geometry.register("flux-denoiser")
class FluxDenoiser(BaseModule):
    ...
    class Config(BaseModule.Config):
        ...
        use_shape_condition: bool = False
        label_condition_dim: int = 1024
        shape_condition_dim:int = 64
    ...
    def configure(self) -> None:
        ...
        with_visual_condition = self.cfg.use_visual_condition and self.cfg.visual_condition_dim != self.cfg.condition_dim
        with_label_condition = self.cfg.use_label_condition and self.cfg.label_condition_dim != self.cfg.condition_dim
        with_caption_condition = self.cfg.use_caption_condition and self.cfg.caption_condition_dim != self.cfg.condition_dim
        with_shape_condition = self.cfg.use_shape_condition and self.cfg.shape_condition_dim != self.cfg.condition_dim
        if with_visual_condition:
        ...
        if with_shape_condition:
            self.proj_shape_condtion = nn.Sequential(
                nn.RMSNorm(self.cfg.shape_condition_dim),
                nn.Linear(self.cfg.shape_condition_dim, self.cfg.condition_dim),
            )
        ...
        if self.cfg.pretrained_model_name_or_path:
            local_model_path = f"{smart_load_model(self.cfg.pretrained_model_name_or_path, self.cfg.subfolder)}/transformer/diffusion_pytorch_model.safetensors"
            print(f"Loading pretrained FluxDenoiser model from {local_model_path}")
            pretrain_safetensors = load_file(local_model_path)
            modules_state_dicts = {}
            for k in pretrain_safetensors.keys():
                k_lst = k.split('.')
                mod = k_lst[0]
                k_name = '.'.join(k_lst[1:])
                if mod not in modules_state_dicts.keys():
                    modules_state_dicts[mod] = dict()
                modules_state_dicts[mod][k_name] = pretrain_safetensors[k]
            # load each module separately, enable grad only for modules not loaded from pretrain_safetensors
            if "dit_model" in modules_state_dicts.keys():
                print(f"Loading DiT Model weights")
                self.dit_model.load_state_dict(modules_state_dicts["dit_model"], strict=True)
            if with_visual_condition and "proj_visual_condtion" in modules_state_dicts.keys():
                print(f"Loading Visual projection weights")
                self.proj_visual_condtion.load_state_dict(modules_state_dicts["proj_visual_condtion"], strict=True)
            if with_label_condition and "proj_label_condtion" in modules_state_dicts.keys():
                print(f"Loading Label projection weights")
                self.proj_label_condtion.load_state_dict(modules_state_dicts["proj_label_condtion"], strict=True)
            if with_caption_condition and "proj_caption_condtion" in modules_state_dicts.keys():
                print(f"Loading Caption projection weights")
                self.proj_caption_condtion.load_state_dict(modules_state_dicts["proj_caption_condtion"], strict=True)
            if with_shape_condition and self.proj_shape_condtion and "proj_shape_condtion" in modules_state_dicts.keys():
                print(f"Loading shape projection weights")
                self.proj_shape_condtion.load_state_dict(modules_state_dicts["proj_shape_condtion"], strict=True)
    ...
    def forward(
        ...
        shape_condition: Optional[torch.FloatTensor] = None,
        ...
        Args:
            ...
            shape_condition (torch.FloatTensor): [bs, n_data, c]
        ...
        condition = []
        if self.cfg.use_visual_condition and visual_condition is not None:
        ...
        if self.cfg.use_shape_condition:  
            if self.cfg.shape_condition_dim != self.cfg.condition_dim:
                shape_condition = self.proj_shape_condtion(shape_condition)
            condition.append(shape_condition)
