...
def add_positional_encoding(hidden_states, max_views=6):
    """
    Add sinusoidal positional encoding to multi-view hidden states.
    
    Args:
        hidden_states: Tensor of shape (num_views, seq_len, hidden_dim)
        max_views: Maximum number of views for normalization (default 12)
    
    Returns:
        Tensor with positional encoding added
    """
    num_views, seq_len, hidden_dim = hidden_states.shape

    device = hidden_states.device
    dtype = hidden_states.dtype
    
    # Create position indices for each view (normalized by max_views)
    positions = torch.arange(num_views, device=device, dtype=dtype).unsqueeze(1)  # (num_views, 1)
    # Create dimension indices for sinusoidal encoding
    dim_indices = torch.arange(0, hidden_dim, 2, device=device, dtype=dtype)  # (hidden_dim/2,)
    div_term = torch.exp(dim_indices * (-math.log(10000.0) / hidden_dim))  # (hidden_dim/2,)
    
    # Compute sinusoidal positional encoding
    pe = torch.zeros(num_views, hidden_dim, device=device, dtype=dtype)
    pe[:, 0::2] = torch.sin(positions * div_term)  # even indices
    pe[:, 1::2] = torch.cos(positions * div_term)  # odd indices
    
    # Expand to match sequence dimension: (num_views, 1, hidden_dim) -> broadcasts over seq_len
    pe = pe.unsqueeze(1)
    return hidden_states + pe
...
class IG2MVSDXLPipeline(StableDiffusionXLPipeline, CustomAdapterMixin):
    ...
    def prepare_control_image(
        ...
        if do_classifier_free_guidance:
            ...
            else:  # separate cfg for input mv and cond im
    ...
    def __call__(
        ...
        original_shape_mv: Optional[PipelineImageInput] = None,
        reference_conditioning_scale: Optional[float] = 1.0,
        cfg_type: str = "single",
        guidance_scale_img: float = 5.0, 
        guidance_scale_src_mv: float = 5.0,
        **kwargs,
    ):
        r"""
        ...
        if self.do_classifier_free_guidance:
            if cfg_type == "single":
                ...
                add_text_embeds = torch.cat(
                    [negative_pooled_prompt_embeds, negative_pooled_prompt_embeds, add_text_embeds], dim=0
                )
        ...
        b_samples = reference_latents.shape[0]
        original_shape_mv = self.image_processor.preprocess(original_shape_mv)
        num_views = original_shape_mv.shape[0]
        original_shape_latents = self.prepare_image_latents(
            original_shape_mv,
            timesteps[:1].repeat(batch_size * num_images_per_prompt * num_views),  # no use
        ...
        all_cross_attention_latents = torch.cat([reference_latents, original_shape_latents], dim=0)
        bs = all_cross_attention_latents.shape[0]
        with torch.no_grad():
            ref_timesteps = torch.zeros(bs, device=device, dtype=timesteps.dtype)
            ref_hidden_states = {}

            self.unet(
                all_cross_attention_latents,
                ref_timesteps,
                encoder_hidden_states=prompt_embeds[-1:].repeat_interleave(bs, dim=0),
                added_cond_kwargs={
                    "text_embeds": add_text_embeds[-1:].repeat_interleave(bs, dim=0),
                    "time_ids": add_time_ids[-1:].repeat_interleave(bs, dim=0),
                },
            ...
            for k, v in ref_hidden_states.items():
                ...
                v_ref_image_repeat = v_ref_image.repeat_interleave(num_views, dim=0)
                #  add positional encoding to the original mv
                v_original_mv = add_positional_encoding(v_original_mv, num_views)
        ...
        if self.do_classifier_free_guidance:
            if cfg_type == "single":
                ...
                for k, v in ref_hidden_states.items():
                    n_latents = v.shape[-2] // 2
                    uncond = torch.zeros_like(v)
                    ref_image_only_cond = v.clone()
                    ref_image_only_cond[:, n_latents:] = 0 # zero out the original mv part
                    cond = torch.cat([uncond, ref_image_only_cond, v], dim=0)
                    ref_hidden_states[k] = cond
                ...
                if self.do_classifier_free_guidance:
                    if cfg_type == "single":
                        ...
                        noise_pred = noise_pred_uncond + guidance_scale_img * (
                            noise_pred_img - noise_pred_uncond
                        ) + guidance_scale_src_mv * (
                            noise_pred_both - noise_pred_uncond
                        )
        ...
        if not output_type == "latent":
            ...
            vae_decode_chunk_size = 1  # Decode 2 views at a time
            images = []
            for i in range(0, latents.shape[0], vae_decode_chunk_size):
                chunk = self.vae.decode(
                    latents[i:i+vae_decode_chunk_size], 
                    return_dict=False
                )[0]
                images.append(chunk)
