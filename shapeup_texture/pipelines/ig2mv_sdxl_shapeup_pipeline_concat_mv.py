...
class IG2MVSDXLPipeline(StableDiffusionXLPipeline, CustomAdapterMixin):
        ...
        **kwargs,
        ...
        if self.do_classifier_free_guidance:
            if cfg_type == "single":
                ...
                add_text_embeds = torch.cat(
                    [negative_pooled_prompt_embeds, add_text_embeds, add_text_embeds], dim=0
                )
        ...
        with torch.no_grad():
            ...
            ref_hidden_states = {}
            modified_conv_in = self.unet.conv_in
            original_in_channels = self.unet.config.in_channels
            self.unet.conv_in = self.unet.original_convolutional_in
            ...
            try:
                ...
                self.unet.conv_in = modified_conv_in
                self.unet.config.in_channels = original_in_channels
        ...
        if self.do_classifier_free_guidance:
            if cfg_type == "single":
                ...
                ref_hidden_states = {
                    k: torch.cat([torch.zeros_like(v), v, v], dim=0)
                    for k, v in ref_hidden_states.items()
                }
            ...
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    if cfg_type == "single":
                        ...
                        conditioning_latents = torch.cat([torch.zeros_like(original_shape_latents), original_shape_latents], dim=0)
                    else: # separate cfg for input mv and cond im 
                        latent_model_input = torch.cat([latents] * 3)
                        conditioning_latents = torch.cat([torch.zeros_like(original_shape_latents), torch.zeros_like(original_shape_latents), original_shape_latents], dim=0)
                else:
                    latent_model_input = latents
                    conditioning_latents = original_shape_latents


                print("latent_model_input.shape", latent_model_input.shape)
                print("conditioning_latents.shape", conditioning_latents.shape)

                # concat original_shape_latents to latent_model_input
                latent_model_input = torch.cat([latent_model_input, conditioning_latents], dim=1) #[12 ,8 , 96, 96]
                ...
                if self.do_classifier_free_guidance:
                    if cfg_type == "single":
                        ...
                        noise_pred_uncond, noise_pred_image, noise_pred_both = noise_pred.chunk(3)
    ...
    def _init_custom_adapter(
        ...
        # ============ NEW: Modify conv_in for 8 channels ============
        if unet_input_channels != 4:
            import torch.nn as nn
            
            original_conv = self.unet.conv_in
            self.original_convolutional_in = original_conv
            self.unet.add_module("original_convolutional_in", original_conv) 
            ...
            new_conv = nn.Conv2d(
                ...
                bias=(original_conv.bias is not None),  # Match original bias settin
            )

            # Initialize weights: copy original 4 channels, zero-init additional channels
            with torch.no_grad():
                # Copy weights for original 4 channels
                new_conv.weight[:, :4, :, :].copy_(original_conv.weight)
                # Zero-initialize the new channels (4-8)
                new_conv.weight[:, 4:, :, :].zero_()
                # Copy bias
                if original_conv.bias is not None:
                    new_conv.bias.copy_(original_conv.bias)

            new_conv = new_conv.to(device=original_conv.weight.device, dtype=original_conv.weight.dtype)
            # Replace conv_in
            self.unet.conv_in = new_conv
            self.unet.config.in_channels = unet_input_channels
            print(f"✓ Modified UNet conv_in: 4 → {unet_input_channels} input channels")
            print(f"  Weight dtype: {new_conv.weight.dtype}, Bias dtype: {new_conv.bias.dtype if new_conv.bias is not None else 'None'}")
            
        else:
            self.original_convolutional_in = None
        # ============================================================
