...
class IG2MVSDXLPipeline(StableDiffusionXLPipeline, CustomAdapterMixin):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print(f'in pipe prompt2: {prompt_2}')
        if prompt is not None and isinstance(prompt, str):
            print(f'in pipe prompt: {prompt}')
            batch_size = 1
        elif prompt is not None and isinstance(prompt, list):
            print(f'in pipe prompt list: {prompt}')
        ...
        print(f'add_time_ids args: {original_size}, {crops_coords_top_left}, {target_size}, {prompt_embeds.dtype}, {text_encoder_projection_dim}')
        ...
        print(f'in pipe reference_image: {reference_image.shape}')
        ...
        print(f'in pipe reference_latents: {reference_latents.shape}')
        print(f'in pipe prompt_embeds: {prompt_embeds.shape}')
        print(f'in pipe add_text_embeds: {add_text_embeds.shape}')
        print(f'in pipe add_time_ids: {add_time_ids.shape}')
        print(f'in pipe add_time_ids: {add_time_ids}')
        with torch.no_grad():
            ...
            for k, v in ref_hidden_states.items():
                print(f'in pipe ref_hidden_states[{k}]: {v.shape}')
        if self.do_classifier_free_guidance:
            ref_hidden_states = {
            ...
            for k, v in ref_hidden_states.items():
                print(f'in pipe ref_hidden_states[{k}] after classifier free guidance: {v.shape}')
        ...
        print(f'control_image_feature: {control_image_feature.shape}')
        ...
        for i, state in enumerate(adapter_state):
            ...
            print(f'adapter_state[{i}]: {adapter_state[i].shape}')
        ...
        print(f'control_conditioning_factor: {control_conditioning_factor}')
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i < int(num_inference_steps * control_conditioning_factor):
                    ...
                    print(f'in pipe down_intrablock_additional_residuals: {len(down_intrablock_additional_residuals)}')
                else:
        ...
        if not output_type == "latent":
            ...
            print(f'final latents before vae decode: {latents.shape}')
            vae_decode_chunk_size = 2  # Decode 2 views at a time
            images = []
