...
from ..pipelines.ig2mv_sdxl_shapeup_pipeline import IG2MVSDXLPipeline
...
class IG2MVSDXLSystem(BaseSystem):
    ...
    class Config(BaseSystem.Config):
        ...
        # Set these to pull the base adapter from the Hub instead of a local file.
        pretrained_adapter_subfolder: Optional[str] = None
        pretrained_adapter_weight_name: Optional[str] = None
        ...
        original_shape_drop_prob: float = 0.0
        ...
        eval_guidance_scale_img: float = 1.0
        eval_guidance_scale_src_mv: float = 1.0
        eval_height: int = 512
        eval_width: int = 512
    ...
    def configure(self):
        ...
        if self.cfg.pretrained_adapter_name_or_path:
            ...
            adapter_name = self.cfg.pretrained_adapter_weight_name
            if adapter_name is None:
                # A path to the file itself: split it into directory + file name.
                pretrained_path = os.path.dirname(pretrained_path)
                adapter_name = os.path.basename(
                    self.cfg.pretrained_adapter_name_or_path)
            pipeline.load_custom_adapter(
                pretrained_path, weight_name=adapter_name,
                subfolder=self.cfg.pretrained_adapter_subfolder)
    ...
    def forward(
        self,
        noisy_latents: Tensor, # torch.Size([num_views * bs, 4, 96, 96])
        conditioning_pixel_values: Tensor, #torch.Size([6, 6, 768, 768])
        timesteps: Tensor, #torch.Size([num_views * bs])
        ref_latents: Tensor, #torch.Size([bs, 4, 96, 96])
        original_shape_latents: Tensor, #torch.Size([bs * num_views, 4, 96, 96])
        prompts: List[str], # [""]
        num_views: int, # 6
        **kwargs,
    ) -> Dict[str, Any]:
        ...
        original_mv_drop_mask = (
            torch.rand(b_samples * num_views, device=noisy_latents.device)
            < self.cfg.original_shape_drop_prob
        ) # torch.Size([bs * num_views])

        prompt_drop_mask = prompt_drop_mask | cond_drop_mask
        image_drop_mask = image_drop_mask | cond_drop_mask
        original_mv_drop_mask = original_mv_drop_mask | cond_drop_mask.repeat_interleave(num_views)
        ...
        all_cross_attention_latents = torch.cat([ref_latents, original_shape_latents], dim=0) # [bs * 7, 4, 96, 96]
        # Process reference latents to obtain reference features
        with torch.no_grad():
            # ref_timesteps = torch.zeros_like(timesteps[:b_samples])
            bs = all_cross_attention_latents.shape[0]
            ref_timesteps = torch.zeros(bs, device=timesteps.device, dtype=timesteps.dtype)
            ref_hidden_states = {}
            self.unet(
                # ref_latents,
                all_cross_attention_latents,
                ref_timesteps,
                encoder_hidden_states=additional_embeds["prompt_embeds"][-1:].repeat_interleave(bs, dim=0),
                added_cond_kwargs={
                    "text_embeds": additional_embeds["text_embeds"][-1:].repeat_interleave(bs, dim=0),
                    "time_ids": additional_embeds["time_ids"][-1:].repeat_interleave(bs, dim=0),
                },
            ...
            for k, v in ref_hidden_states.items():
                ...
                v_ref_image = v[:b_samples]
                v_original_mv = v[b_samples:]
                v_ref_image[image_drop_mask] = 0.0
                v_original_mv[original_mv_drop_mask] = 0.0
                v_ref_image_repeat = v_ref_image.repeat_interleave(num_batch_images, dim=0)
                ref_hidden_states[k] = torch.cat([v_ref_image_repeat, v_original_mv], dim=-2)
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=False):
            original_shape_latents = []
            for i in range(0, batch["src_rgb"].shape[0], vae_max_slice):
                original_shape_latents.append(
                    vae_encode(
                        self.vae,
                        batch["src_rgb"][i : i + vae_max_slice].to(self.vae.dtype) * 2 - 1,
                        sample=True,
                        apply_scale=True,
                    ).float()
                )
            original_shape_latents = torch.cat(original_shape_latents, dim=0)
        ...
        model_pred = self(
            noisy_latents, batch["source_rgb"], timesteps, ref_latents, original_shape_latents, **batch
        )["noise_pred"]
    ...
    def get_output_visualizations(self, batch, outputs):
        ...
        for i in range(len(images)):
            print(f'images[{i}].shape: {images[i]["img"].shape}')
        return images

    def get_output_visualizations_new(self, batch, outputs, train=True):
        if train:
            images = [
                {
                    "type": "rgb",
    ...
    def generate_images(self, batch, **kwargs):
        return self.pipeline(
            ...
            guidance_scale_img=self.cfg.eval_guidance_scale_img,
            guidance_scale_src_mv=self.cfg.eval_guidance_scale_src_mv,
            cfg_type=self.cfg.cfg_type,
            height=self.cfg.eval_height,
            width=self.cfg.eval_width,
            reference_image=batch["reference_rgb"],
            original_shape_mv=batch["src_rgb"],
            output_type="pt",
            
        ).images
    
    def generate_images_validation(self, batch, **kwargs):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if (
            ...
            fig = self.get_output_visualizations_new(batch, out, train=False)
            wandb_logger = self.loggers[2]
            self.save_image_grid(
                f"it{self.true_global_step}-validation-{batch['uid'][0]}.jpg",
                fig,
                name=f"{batch['uid'][0]}_validation_{self.global_rank}_{batch_idx}",
                step=self.true_global_step,
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.save_image_grid(
            ...
            self.get_output_visualizations_new(batch, out, train=False),
